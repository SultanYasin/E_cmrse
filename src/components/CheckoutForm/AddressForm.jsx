...
const AddressForm = ({ checkoutToken, test }) => {
  ...
  const fetchShippingCountries = async (checkoutTokenId) => {
    ...
    setShippingCountry(Object.keys(countries)[0]);// convert the object to an array and get the first element
  };
  ...
  // after that I'm able to get the ocntries that I can ship to
  useEffect(() => {
    fetchShippingCountries(checkoutToken.id);
  }, []);
  // if shipping country is exists then recall fetchShippingSubdivisions
  useEffect(() => {
    if (shippingCountry) fetchSubdivisions(shippingCountry);
  }, [shippingCountry]);
  // if fetchShippingSubdivisions is exists then recall fetchShippingOptions
  useEffect(() => {
